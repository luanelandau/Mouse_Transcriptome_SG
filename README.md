...
6. Generate the quantitative source plots for Figures 1–3 (`Figure_1/`, `Figure_2/`, and `Figure_3/`).
7. Analyze genomic clustering, promoter motifs, and TAD structure (`cluster_analysis/` and `TAD_analysis/`); the TAD analyses contribute to Figure 4.
...
The TAD component of **Figure 4** is under `TAD_analysis/`. The main workflow uses `TAD_analysis/hic_25kb.sh` with `TAD_analysis/CH12_25kb_select.ini`. The nested `TAD_analysis/TADs_sex_differences/` workflow compares male and female TADs using `hic_25kb_new.sh` and `ser_gra_svg_25kb_new.ini`.

The accompanying chromatin tracks were obtained as processed bigWig files from ENCODE and include ATAC-seq, CTCF, RAD21, and H3K4me3 datasets. See [`TAD_analysis/README.md`](TAD_analysis/README.md) and [`TAD_analysis/TADs_sex_differences/README.md`](TAD_analysis/TADs_sex_differences/README.md) for accessions and analysis notes.
...
| [`TAD_analysis/`](TAD_analysis/) | Figure 4 TAD workflows, including the nested male–female comparison; see [`TAD_analysis/README.md`](TAD_analysis/README.md) |
