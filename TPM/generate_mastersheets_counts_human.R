...
dir_hum <- "~/Library/CloudStorage/Box-Box/SalivaryGlands_LL/Mouse_Transcriptome_SG/readCounts_human/"

files_counts <- c(
  list.files(dir_hum, pattern = "\\.counts\\.txt$", full.names = TRUE)
)
files_counts <- files_counts[!grepl("counts_with_tpm\\.txt$", files_counts)]  # extra guard

files_counts_with_tpm <- c(
  list.files(dir_hum, pattern = "\\.counts_with_tpm\\.txt$", full.names = TRUE)
)
...
## ---------------- Write ----------------
out_counts <- "miscelaneous_sheets/gene_expression_matrix_human_RAW_COUNTS.csv"
out_tpm    <- "miscelaneous_sheets/gene_expression_matrix_human_TPMs.csv"
